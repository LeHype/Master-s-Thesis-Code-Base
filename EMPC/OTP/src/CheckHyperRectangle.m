...
% Add necessary path for CasADi functions
try
...
% Due to parallel computing, the function has to be re-constructed for some threads
if isempty(f)
...
for i = 0:(num_vertices - 1)
    ...
    % Get OCP solution. CachedF calculates it and stores the solution in a hashmap so it does not have to be recomputed
...
if any(isnan(V_star_all)) % Any vertex infeasible
    feasibility = 0;
    Message = 'Infeasible Vertex found';
end

if ~any(~isnan(u_star_all), 'all')
    % All vertices infeasible, possible infeasible region. If midpoint also infeasible --> region is deemed infeasible
    middle_point = lower_bound + ((upper_bound - lower_bound) / 2); % Check midpoint if infeasible
    try
        f(middle_point);
    catch
        Message = 'Infeasible region found';
  
    end
end

% Start epsilon calculation for region
...
u_star_inner = zeros(num_vertices + 1, size_u_Star(1) * size_u_Star(2)) * NaN; % Adjust 'size_u_Star' as necessary
...
% Next split according to heuristic splitting rule
...
% Calculate OCP solution. If qrqp fails, sometimes ipopt finds a feasible solution.
