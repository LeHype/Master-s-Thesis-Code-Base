...
nSteps   = 20;
uMax            = 0.82;        % input voltage is limited !When using dry friction compensation this needs to be reduced accordingly!

% Hardcode l_bar Adjust for each scenario
l_bar = 0.4;
...
x=[x0_p, x_dec]; 
for Node = 1:nSteps-1
    x(:,Node+1) = ode_fun_Y(x(:,Node), u(:, Node));
end
...
ref_tracking_error = wQ(1)*sum(sum((x(1,:)-x_Y_ref(1,:)).^2));
ref_tracking_error =ref_tracking_error + wQ(2)*sum(sum((x(2,:)-x_Y_ref(2,:)).^2));
ref_tracking_error =ref_tracking_error + wQ(3)*sum(sum((x(3,:)-x_Y_ref(3,:)).^2));
ref_tracking_error =ref_tracking_error + wQ(4)*sum(sum((x(4,:)-x_Y_ref(4,:)).^2));
ref_tracking_u_error =sum(sum((u(1,:)-u_Y_ref(1,:)).^2));
...
upper_bound = [0.6,  pi/5, 0.5,  -pi/2.5];
